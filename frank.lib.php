...
class Frank_InvalidMiddlewareClass extends Exception {}
/** File not found exception **/
class Frank_CallbackFileNotFoundException extends Exception {}
/** Invalid callback exception **/
class Frank_InvalidCallbackException extends Exception {}
/** Invalid URI Parameter exception **/
class Frank_InvalidURIParameterException extends Exception {}
/** Invalid State of HTTP Response exception **/
class Frank_InvalidResponseStateException extends Exception {}
/** Invalid HTTP Response Code exception **/
class Frank_InvalidResponseCodeException extends Exception {}
...
* @see: http://frank.org/doc.html#Pre-defined_Validator_Types
*/
final class Frank_Constants {
...
class Frank_Callback_Util {
  ...
  private static function loadFile($file) {
    ...
    if (file_exists($file)) {
      ...
      throw new Frank_CallbackFileNotFoundException('Controller file not found');
    }
  ...
  public static function getCallback($callback, $file = null) {
    ...
    throw new Frank_InvalidCallbackException('Invalid callback');
  ...
  public function pattern($token, $pattern = null) {
    ...
    if ($pattern) {
      ...
      if (isset($this->patterns[$token])) {
        ...
        $pattern = Frank_Constants::PATTERN_ANY;
      }
      
      if ((is_string($pattern) && is_callable($pattern)) || is_array($pattern)) {
        $this->callbacks[$token] = $pattern;
        $this->patterns[$token] = $pattern = Frank_Constants::PATTERN_ANY;
      }
        ...
        foreach($matches as $k => $v) {
          if (is_numeric($k)) {
            ...
            if (isset($this->callbacks[$k])) {
              $callback = Frank_Callback_Util::getCallback($this->callbacks[$k]);
              $value    = call_user_func($callback, $v);
              if ($value) {
                $matches[$k] = $value;
              } else {
                throw new Frank_InvalidURIParameterException('Invalid parameters detected');
              }
            }
...
class Frank_Response {
  ...
  */  
  public function flush($code = null, $format = null) {
    ...
    if (!empty($code)) { 
      if (headers_sent()) {
        throw new Frank_InvalidResponseStateException("Response code already sent: {$this->code}");
      }
      ...
      if (array_key_exists($code, $codes)) {
        ...
        throw new Frank_InvalidResponseCodeException("Invalid Response Code: $code");
      }
    }
    ...
    if (!empty($format)) { 
      if (headers_sent()) {
        throw new Frank_InvalidResponseStateException("Response format already sent: {$this->format}");
      }
    ...
    foreach (Frank_Router::$middleware as $m) {
...
class Frank_Request {
  ...
  function __construct() {
    ...
    $this->method = Frank_Router::getRequestMethod();    
...
abstract class Frank_Middleware {
...
class Frank_Router {
  ...
  */
  public function addRoute($params) {
    ...
    if (!empty($params['path'])) {
      ...
      $template = new Frank_Template($params['path']);
      ...
      foreach ($methods as $method) {
        ...
        Frank_Middleware::$routes[$method][$params['path']] = $this->routes[$method][$params['path']];
      }
  ...
  */
  public function attach() {
    ...
    if (!is_subclass_of($className, 'Frank_Middleware')) {
      throw new Frank_InvalidMiddlewareClass("Middleware class: '$className' does not exist or is not a sub-class of Frank_Middleware" );
    }
  ...
  */
  ...
  public function route($uri = null) {
    if (empty($uri)) {
      ...
      // Using parse_url with URI can cause bugs like this: https://github.com/frank/frank/issues/13
    ...
    foreach ($routes as $route) {
      ...
      if (!is_null($params)) {
        Frank_Middleware::$context['pattern'] = $route['template']->getTemplate();
        Frank_Middleware::$context['http_method'] = self::getRequestMethod();
        Frank_Middleware::$context['callback'] = $route['callback'];
        
        $callback = Frank_Callback_Util::getCallback($route['callback'], $route['file']);
        return $this->invoke_callback($callback, $params);
      }
    }
    
    if (strcasecmp(Frank_Router::getRequestMethod(), "options") == 0)
    {
        return $this->invoke_options();
    }
    
    throw new Frank_InvalidPathException('Invalid path');
  }
  ...
  */
  protected function invoke_callback($callback, $params) {
    ...
    $req = new Frank_Request();
    $req->params = $params;
    $res = new Frank_Response($req);
  ...
  protected function invoke_options() {
      ...
      header("Allow: " . implode(",", array_map('strtoupper',Frank_Router::$methods)));
      $res->send(200);
